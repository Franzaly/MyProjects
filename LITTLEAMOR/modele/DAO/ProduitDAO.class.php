...
class ProduitDAO implements DAO
{
	...
	public static function chercher($cles)
	{
		...
		$requete = $connexion->prepare("SELECT * FROM produit WHERE code=? OR description=?");
		// Exécuter la requête
	...
	static function inserer($unProduit)
	{
		...
		$requete = $connexion->prepare("INSERT INTO produit (code,description,prix,quantite,url_photo) VALUES (?,?,?,?,?,?)");
	...
	static public function modifier($unProduit)
	{
		...
		$requete = $connexion->prepare("UPDATE produit SET description=?,prix=?,quantite=?,url_photo=? WHERE code=?");
